...
async def main():
  ...
  try:
    ...
    if pending_idx != -1:
      ...
      print('🧹 queue.json updated locally.')

      queue_data, queue_sha = get_github_file(QUEUE_FILE)
      if queue_sha:
        update_github_file(
            QUEUE_FILE,
            links_data,
            queue_sha,
            f'Mark index {pending_idx + 1} as done',
        )
        print('☁️ queue.json successfully updated on GitHub via API!')
